...
int main(int argc, char **argv)
{
	...
	void *buffer = mmap(NULL, BUF_SIZE, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);
	...
	printf("pin cpu core %d in monitor\n", core_ID);
	//pin_cpu(core_ID);
        
	printf("sched_getcpu = %d\n", sched_getcpu());
	//cpu_set_t my_set;        /* Define your cpu_set bit mask. */
  	//CPU_ZERO(&my_set);       /* Initialize it all to 0, i.e. no CPUs selected. */
  	//CPU_SET(core_ID, &my_set);     /* set the bit that represents core 7. */
	//sched_setaffinity(20, sizeof(cpu_set_t), &my_set); 

  	//if (sched_setaffinity(0, sizeof(cpu_set_t), &my_set) == -1) /* Set affinity of tihs process to the defined mask, i.e. only 7. */
	//    errExit("sched_setaffinity");
	//setpriority(PRIO_PROCESS, 0, -20);
	//printf("sched_getcpu = %d\n", sched_getcpu());
	//printf("pin cpu in monitor00\n");
	...
	*current = monitoring_set;
	...
	for (int victim_iteration_index = 1; victim_iteration_index < 3; victim_iteration_index++) {
		...
		for (rept_index = 0; rept_index < repetitions; rept_index++) {
			...
			int temp_out = sprintf(output_data_fn, "./out/%04d_data_%d.out", rept_index, victim_iteration_index);
			FILE *output_data;
			//fprintf(stderr, "%d, %s, READY3\n", temp_out, output_data_fn);
			output_data = fopen(output_data_fn, "w+");
			if (output_data == NULL) {
    				fprintf(stderr, "can't open %s: %s\n", output_data_fn, strerror(errno));
    				exit(1);
			}
